...
class CorpusParser:
    ...
    def parse(
        ...
        for path in discovered_files:
            ...
            page_texts = self._split_pages(text)
            elements = self._extract_semantic_elements(
    ...
    def _extract_text(self, path: Path) -> str:
        """Extract text from a file using the configured extractor factory."""
    ...
    def _split_pages(self, text: str) -> list[str]:
        """Split document text into page-like units using form-feed delimiter."""
    ...
    def _extract_semantic_elements(
        ...
        """Extract ordered semantic elements from page text blocks."""
        ...
        for page_number, page_text in enumerate(page_texts, start=1):
            for line in page_text.splitlines():
                ...
                if not saw_title:
                    _, normalized_title = self._parse_heading(raw, suffix)
                    order += 1
                    element = SemanticElement(
                        element_id=self._build_element_id(doc_id, order),
                        element_type="title",
                        text=normalized_title or raw,
                        page=page_number,
                        order=order,
                ...
                element_type = self._classify_line(raw, suffix=suffix, in_references=in_references)
                heading_level, heading_text = self._parse_heading(raw, suffix)
                if element_type == "section_heading" and heading_text:
                    section_path = self._update_section_path(section_path, heading_level, heading_text)
                ...
                element = SemanticElement(
                    ...
                    order=order,
                    section_path=tuple(section_path),
                    metadata=self._element_metadata(raw, element_type),
                )
                elements.append(element)

        return self._merge_elements(doc_id, elements)

    def _merge_elements(self, doc_id: str, elements: list[SemanticElement]) -> list[SemanticElement]:
        """Merge adjacent line-level elements into richer semantic blocks."""
        if not elements:
            return []
    ...
    def _classify_line(self, line: str, suffix: str, in_references: bool) -> str:
        """Classify a line into semantic element type."""
        lower = line.lower()
        if in_references:
            return "references"
        heading_level, heading_text = self._parse_heading(line, suffix)
    ...
    def _parse_heading(self, line: str, suffix: str) -> tuple[int, str]:
        """Return heading level and normalized heading text when applicable."""
        ...
        heading_like = {"abstract", "introduction", "conclusion", "references"}
        if normalized.lower() in heading_like:
            return 1, normalized

        words = normalized.split()
        has_math_symbols = any(symbol in normalized for symbol in ("=", "+", "-", "*", "/", "^"))
        alpha_words = [word for word in words if re.search(r"[A-Za-z]", word)]
        if (
            words
            and normalized.isupper()
            and len(words) <= 12
            and len(alpha_words) >= 2
            and not has_math_symbols
        ):
            return 1, normalized.title()
        return 0, ""
    ...
    def _looks_like_table(self, line: str) -> bool:
        """Heuristic detection for table-like textual rows."""
        if line.count("|") >= 2:
            return True
        if "\t" in line:
            return True
    ...
    def _looks_like_equation(self, line: str) -> bool:
        """Heuristic detection for equation-like lines."""
        if "=" not in line:
            return False
        math_tokens = {"+", "-", "*", "/", "^", "∑", "∏", "∫", "λ", "σ"}
        return any(token in line for token in math_tokens)

    def _element_metadata(self, line: str, element_type: str) -> dict:
        """Return element-type-specific metadata payload."""
        if element_type == "table":
            return {"rows": [self._parse_table_row(line)]}
        return {}

    def _parse_table_row(self, line: str) -> list[str]:
        """Parse table line into columns with row and column boundaries preserved."""
        if "|" in line:
            return [piece.strip() for piece in line.split("|") if piece.strip()]
        if "\t" in line:
            return [piece.strip() for piece in line.split("\t") if piece.strip()]
        return [piece.strip() for piece in re.split(r"\s{2,}", line.strip()) if piece.strip()]
    ...
    def _infer_authors(self, elements: list[SemanticElement]) -> list[str]:
        """Heuristically infer author lines from first page front matter."""
    ...
    def _serialize_element(self, item: SemanticElement) -> dict:
    ...
    def _serialize_parent(self, item: ParentNode) -> dict:
    ...
    def _serialize_child(self, item: ChildNode) -> dict:
